...
class GenerateBacklogView(views.APIView):
    ...
    def post(self, request, project_id):
        ...
        if serializer.is_valid():
            ...
            proposal_data = client.generate_backlog_from_description(description)
...
class PrioritizeBacklogView(views.APIView):
    ...
    def get(self, request, project_id):
        """Genera una sugerencia de priorización con IA."""
        project = get_object_or_404(Project, id=project_id)
        self.check_object_permissions(request, project)
        ...
        tasks_list = [dict(t, id=str(t['id'])) for t in tasks]
        ...
        if not suggestion:
            return response.Response({"error": "La IA no pudo generar una sugerencia válida."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
